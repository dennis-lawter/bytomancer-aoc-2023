use super::final_answer;
use super::input_raw;

const DAY: u8 = 7;

#[derive(Debug, Clone, Copy, PartialEq, PartialOrd, Eq)]
enum Camel1Card {
    C2 = 0,
    C3 = 1,
    C4 = 2,
    C5 = 3,
    C6 = 4,
    C7 = 5,
    C8 = 6,
    C9 = 7,
    T = 8,
    J = 9,
    Q = 10,
    K = 11,
    A = 12,
}
impl Ord for Camel1Card {
    fn cmp(&self, other: &Self) -> std::cmp::Ordering {
        (*self as u8).cmp(&(*other as u8))
    }
}
impl Camel1Card {
    fn from_char(c: char) -> Option<Self> {
        match c {
            'A' => Some(Camel1Card::A),
            'K' => Some(Camel1Card::K),
            'Q' => Some(Camel1Card::Q),
            'J' => Some(Camel1Card::J),
            'T' => Some(Camel1Card::T),
            '9' => Some(Camel1Card::C9),
            '8' => Some(Camel1Card::C8),
            '7' => Some(Camel1Card::C7),
            '6' => Some(Camel1Card::C6),
            '5' => Some(Camel1Card::C5),
            '4' => Some(Camel1Card::C4),
            '3' => Some(Camel1Card::C3),
            '2' => Some(Camel1Card::C2),
            _ => None,
        }
    }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd)]
enum Camel1HandType {
    HighCard = 0,
    OnePair = 1,
    TwoPair = 2,
    ThreeOfAKind = 3,
    FullHouse = 4,
    FourOfAKind = 5,
    FiveOfAKind = 6,
}

impl Ord for Camel1HandType {
    fn cmp(&self, other: &Self) -> std::cmp::Ordering {
        (*self as u64).cmp(&(*other as u64))
    }
}

#[derive(Debug, PartialOrd, PartialEq, Eq)]
struct Camel1Hand {
    cards: Vec<Camel1Card>,
    bid: u64,
    hand_type: Camel1HandType,
    score: u64,
}
impl Ord for Camel1Hand {
    fn cmp(&self, other: &Self) -> std::cmp::Ordering {
        if self.hand_type == other.hand_type {
            // hands match, go through cards
            for i in 0..5 {
                if self.cards[i] != other.cards[i] {
                    return self.cards[i].cmp(&other.cards[i]);
                }
            }
        }
        self.hand_type.cmp(&other.hand_type)
    }
}
impl Camel1Hand {
    fn from_string(line: &String) -> Self {
        let parts: Vec<String> = line.split(" ").map(str::to_owned).collect();
        let bid = str::parse::<u64>(parts[1].as_str()).unwrap();
        let cards: Vec<Camel1Card> = parts[0]
            .chars()
            .map(|c| Camel1Card::from_char(c).unwrap())
            .collect();

        Self {
            cards: cards.clone(),
            bid,
            hand_type: Self::determine_type(&cards),
            score: 0,
        }
    }
    fn determine_type(cards: &Vec<Camel1Card>) -> Camel1HandType {
        let mut sorted_hand = cards.clone();
        sorted_hand.sort();

        if sorted_hand[0] == sorted_hand[4] {
            Camel1HandType::FiveOfAKind
        } else if sorted_hand[0] == sorted_hand[3] || sorted_hand[1] == sorted_hand[4] {
            Camel1HandType::FourOfAKind
        } else if (sorted_hand[0] == sorted_hand[2] && sorted_hand[3] == sorted_hand[4])
            || (sorted_hand[0] == sorted_hand[1] && sorted_hand[2] == sorted_hand[4])
        {
            Camel1HandType::FullHouse
        } else if sorted_hand[0] == sorted_hand[2]
            || sorted_hand[1] == sorted_hand[3]
            || sorted_hand[2] == sorted_hand[4]
        {
            Camel1HandType::ThreeOfAKind
        } else if (sorted_hand[0] == sorted_hand[1] && sorted_hand[2] == sorted_hand[3])
            || (sorted_hand[0] == sorted_hand[1] && sorted_hand[3] == sorted_hand[4])
            || (sorted_hand[1] == sorted_hand[2] && sorted_hand[3] == sorted_hand[4])
        {
            Camel1HandType::TwoPair
        } else if sorted_hand[0] == sorted_hand[1]
            || sorted_hand[1] == sorted_hand[2]
            || sorted_hand[2] == sorted_hand[3]
            || sorted_hand[3] == sorted_hand[4]
        {
            Camel1HandType::OnePair
        } else {
            Camel1HandType::HighCard
        }
    }
}

async fn input1(example: bool) -> Vec<Camel1Hand> {
    let raw = input_raw(DAY, example).await;
    let lines: Vec<String> = raw
        .lines()
        .map(|item| item.to_owned())
        .filter(|item| item.len() > 0)
        .collect();

    let mut hands = vec![];

    for line in lines {
        hands.push(Camel1Hand::from_string(&line))
    }

    hands
}

pub async fn d07s1(submit: bool, example: bool) {
    let mut hands = input1(example).await;

    let hand_sort_closure = |left: &Camel1Hand, right: &Camel1Hand| -> std::cmp::Ordering {
        // println!("\nTEST: {:?} vs {:?}", left, right);
        if (left.hand_type as u64) == (right.hand_type as u64) {
            for i in 0..5 {
                if left.cards[i] != right.cards[i] {
                    // println!("PER CARD Result: {:?}", left.cards[i].cmp(&right.cards[i]));
                    return left.cards[i].cmp(&right.cards[i]);
                }
            }
        }
        // println!("DEFAULT: {:?}", left.hand_type.cmp(&right.hand_type));
        left.hand_type.cmp(&right.hand_type)
    };

    hands.sort_by(hand_sort_closure);

    for i in 0..hands.len() {
        hands[i].score = hands[i].bid * (i as u64 + 1);
    }

    let mut accum = 0;

    for hand in hands {
        println!("{:?}", hand);
        accum += hand.score;
    }

    final_answer(accum, submit, DAY, 1).await;
}
